...
def crop(img, center, scale, res, rot=0):
    ...
    if not rot == 0:
        # Remove padding
        M = cv2.getRotationMatrix2D((float(center[0]), float(center[1])), rot, 1)
        new_img = cv2.warpAffine(new_img, M, (int(new_img.shape[0]), int(new_img.shape[1])), borderValue=1)
        new_img = new_img[pad:-pad, pad:-pad]

    new_img = im_to_torch(cv2.resize(new_img, res)[:,:,np.newaxis])
    return new_img
